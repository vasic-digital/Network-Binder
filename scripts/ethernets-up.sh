...
for interface in $(ip -o link show | awk -F': ' '{print $2}' | grep -v "lo"); do
    ...
    if echo "$STATUS" | grep "DOWN"; then
        ...
        if echo "$interface" | grep "wlp"; then

            echo "Skipping Wi-Fi interface '$interface'"

        elif echo "$interface" | grep "br-"; then

            echo "Skipping bridge interface '$interface'"

        elif echo "$interface" | grep "docker"; then

            echo "Skipping Docker interface '$interface'"
